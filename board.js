...
function MOVE_PX(src, dst, step) {
    return (
        Math.floor((src * step + dst * (MAX_STEP - step)) / MAX_STEP + 0.5) +
        "px"
    );
}
        ...
        var timer = setInterval(function () {
            if (step == 0) {
                ...
                this_.imgSquares[sqMate].src =
                    this_.images +
                    (this_.pos.sdPlayer == 0 ? "r" : "b") +
                    "km.gif";
                this_.postMate(computerMove);
            } else {
                style.left =
                    xMate + ((step & 1) == 0 ? step : -step) * 2 + "px";
                step--;
            }
        }, 50);
        return;
...
Board.prototype.drawSquare = function (sq, selected) {
    ...
    img.style.backgroundImage = selected
        ? "url(" + this.images + "oos.gif)"
        : "";
};
...
Board.prototype.flushBoard = function () {
    ...
    for (var sq = 0; sq < 256; sq++) {
        if (IN_BOARD(sq)) {
            this.drawSquare(
                sq,
                sq == SRC(this.mvLast) || sq == DST(this.mvLast)
            );
        }
    }
};
